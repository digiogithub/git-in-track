...
| The data model is frozen at `schemaVersion: 1` with a documented 0.x migration path | **partial** | The on-disk layout is frozen at `schema: 1` (`internal/core.SupportedSchema`, `project.yaml:schema`) and the evolution rules are `docs/03-data-model.md` §19. **`gintrack migrate` did not exist at 1.0** — see §5 (it now does, `GIT-US-0143`), and note that no 0.x release was ever published, so there is nothing in the field to migrate from |
...
- **`gintrack migrate` does not exist.** `docs/03-data-model.md` R-EVO-4 specifies it. It
  ...
  published `schema: 1` — but it must exist before `schema: 2`. *Resolved after 1.0 by
  `GIT-US-0143`: `gintrack migrate --to 2` (docs/07 §4.22) ships with schema 2, with the
  upgrade guide [docs/14-upgrading-to-specs.md](./14-upgrading-to-specs.md).*
