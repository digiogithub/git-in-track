...
- [ ] A batch of a thousand items emits a bounded number of progress events.
- [ ] `done` and `failed` are never suppressed by the throttle.
- [ ] `go test -race ./internal/server/...` covers the throttle with a fake clock.
