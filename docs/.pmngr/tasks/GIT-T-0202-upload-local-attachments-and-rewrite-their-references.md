...
What is left: `POST /api/articles/{id}/attachments` as `multipart/form-data` with no JSON content type, the skip when `ArticleAttachments` already lists the same name and size (`Client.ArticleAttachments` exists), and the `httptest` coverage.
