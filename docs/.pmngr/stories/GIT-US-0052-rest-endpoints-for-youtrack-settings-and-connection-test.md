...
- [ ] `GET|PATCH /api/v1/youtrack/settings`, `POST /api/v1/youtrack/test`, `GET /api/v1/youtrack/projects?q=` and `GET /api/v1/youtrack/fields?project=` are mounted under `/api/v1` and require the bearer token.
- [ ] No response, log line or problem document ever contains the token; `hasToken` and `tokenSource` (`env` | `file` | `none`) are reported instead.
- [ ] `PATCH` returns `persisted: false` when the server was started without `ConfigPath`, matching the git settings contract.
- [ ] `POST /api/v1/youtrack/test` distinguishes 401, 403 and 404-from-a-missing-context-path and returns a distinct problem code for each.
- [ ] `GET /api/v1/youtrack/projects` returns at most the configured page of projects and is safe to call on every keystroke (debounced client side, rate-limited server side).
- [ ] `docs/07-cli-and-api.md` documents all five endpoints with request and response examples.
- [ ] `go test -race ./internal/server/...` covers the handlers against an `httptest` YouTrack stub, including the token-redaction assertion.
