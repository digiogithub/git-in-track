...
- [x] `internal/core/model.go` adds `spec` to the closed item-type enum and models the `requirements:` map (`status`, `trace`, `verified`).
- [x] `internal/core/ids.go` accepts `GIT-SP-NNNN` and parses requirement refs `GIT-SP-NNNN.R<n>`; `allocator.go` allocates spec IDs and the next `R<n>` as max+1 within a file, never reusing a number.
