...
- [x] Two specs under `docs/.pmngr/specs/` (created through the tools): the `rev` protocol (stale_revision, conflicts[], empty-conflict no-op, no `*` escape) and ID allocation (next = max+1, permanence, gaps, requirement `R<n>`).
- [x] Every requirement passes the grammar linter with no warnings and has at least one scenario.
- [x] `// Implements:` and `// Verifies:` markers added in `internal/core`, `internal/vault` and `internal/mcp` code and tests; `gintrack spec coverage` shows no `untested` requirement.
- [x] Markers are comment-only changes; `make test` and `make lint` pass.
