...
- [ ] Hits grouped by tier (direct, transitive, candidate with score), each with reason, coverage status and suspect; candidates visually distinct from certain hits.
- [ ] `unavailable` states: whole view in browser-only mode, tiers 2–3 when Pando is missing.
- [ ] Vitest tests for grouping and unavailable states; docs/05 updated.
