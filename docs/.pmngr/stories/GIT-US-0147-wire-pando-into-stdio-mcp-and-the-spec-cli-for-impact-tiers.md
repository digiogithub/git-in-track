...
- [ ] GIT-US-0121 is rebased into the Phase 11 stack, with its `cmd/gintrack/mcp.go` conflicts resolved.
- [ ] Stdio `gintrack mcp` and `gintrack spec impact` build the same Pando client and semantic searcher as `serve`, through one shared constructor, and pass them to the impact seam (`CallGraph` / `Semantic`).
- [ ] Without Pando configured, behaviour is unchanged: tier 1 answers, and tiers 2 and 3 are `unavailable`.
- [ ] Tests with the fake Pando server show tiers 2 and 3 answering over stdio and through the CLI. `make lint`, `make test` and `make wasm` pass.
- [ ] docs/08 and docs/07 are updated.
