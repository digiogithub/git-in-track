...
Everything that reaches an external tracker sits inside the `gintrack` box, and nothing in the
Browser box has an arrow to it. That is not a diagram detail: the whole YouTrack feature —
importing issues, pushing comments, publishing and pulling knowledge-base pages — is
**companion-only**, and the web app learns so from two separate capability flags rather than one.
`youtrackSupported` says whether this *runtime* could ever reach a tracker, and it is false in
browser-only mode; `youtrack` says whether a project is actually linked. The settings card shows on
the first alone, because a card that appeared only once a project was connected could never connect
the first one; the import entry needs both, because importing from an instance nothing is linked to
means nothing. In browser-only mode the feature is therefore absent rather than broken — no button,
no card, no error — which is the only honest rendering of a capability the runtime does not have.
...
| External trackers | None. A tab has no process to hold a credential and no way past a tracker's CORS policy, so the YouTrack integration is hidden rather than degraded |
...
| MCP | `gintrack mcp` over stdio, and `POST /mcp` on the local server with `--mcp-http`; the same tools on both — 7 read-only, 22 with `--allow-write` — read-only by default |
| Background jobs | `internal/syncengine`: a journalled queue with a worker pool, started and stopped with the server, carrying every outbound call to a tracker |
| YouTrack | The companion holds the token and makes the call; the browser never talks to the instance ([ADR-032](adr/ADR-032-local-integration-credential-storage.md)) |
| Browser git networking | The companion's CORS proxy at `/cors-proxy/`, adopted automatically by a tab that has detected the companion; otherwise a proxy the user configures ([ADR-025](adr/ADR-025-the-cors-proxy-security-model.md)) |

The companion runs four things beside the HTTP listener, and the background job engine
(`internal/syncengine`) is the newest of them: the fsnotify watcher, the debounced commit-on-save
committer, the optional quick tunnel, and the engine. It is built by `server.New`, brought up in
`Server.Start` on the same path as the other three and closed on the way out of it with a context
detached through `context.WithoutCancel` — because a job halfway through a call to somebody else's
tracker must be allowed to finish, and the very shutdown waiting for it must not be what cancels it.
Shutdown then drains for a bounded grace period (five seconds by default) and journals whatever is
left, which runs on the next start: a process that will not exit is worse than a job that runs
again. Handlers are registered before `Start`, because `Start` replays the journal and a replayed
job whose kind has nothing registered is work the engine can never do. With no integration
configured the engine still starts, and simply stays idle.
...
internal/trace/          # marker scanner + requirement trace graph (native-only)
internal/impact/         # three-tier requirement impact of a diff (native-only)
internal/youtrack/       # YouTrack REST client + the issue/article mapper (native-only)
internal/syncengine/     # background job queue (native-only)
...
| `internal/vault/` | The CoreApi contract of `web/src/core-bridge/api.ts` implemented once, over a `core.FS` the host injects: `NewInMemory()` for the browser (files pushed in with `vault.load`), `Open(fsys, root)` for the companion process (files read through `internal/core/osfs`). Exposes `Call` (JSON envelope, for the WASM glue) and `Dispatch` (typed result and error, for the REST layer). `Dispatch` holds the vault mutex for the whole call, so a query never observes a half-applied write, with two exceptions that must never run under it: the YouTrack methods take it only around their index reads and writes, and the reads answered by a host-installed seam — `trace.requirement`, `trace.touching`, `coverage.list`, `spec.context`, `impact.query`, `impact.report` — take it only to refresh and capture the index, then release it before the tracer, the coverage backend or the impact resolver runs (GIT-US-0163). Those seams read git and the working tree and, for impact tiers 2 and 3, call Pando, whose semantic searcher resolves its hits back through the vault's own `Requirement`, `Item` and `Page`: under the mutex that call back deadlocked. The index they read is safe for concurrent use and hands out copies; a write landing mid-call may be seen by some of the seam's reads and not others. The one seam call that stays under the mutex is `RequirementCoverage.StampEvidence`, because a verification stamp is decided and written in one transaction; it must not call back into the vault. **Must not import** `os`, `path/filepath` or `syscall/js`. |
| `internal/server/` | chi router, REST handlers, WebSocket hub, static file serving of the embedded `web/dist`, localhost binding, token middleware, origin checks. Translates core errors into HTTP status codes. It also mounts the git CORS proxy at `/cors-proxy/` (`cors_proxy.go`), which is what lets browser-only mode reach a git host at all: a deliberately narrow forwarder — three git smart-HTTP paths, an allow-list of hosts derived from the registered repositories' remotes, public unicast addresses only, bounded and re-validated at every redirect ([ADR-025](adr/ADR-025-the-cors-proxy-security-model.md), docs/06 §6.3). Native-only, like the rest of this package. |
...
| `internal/trace/` | The marker scanner of the requirement trace engine (GIT-US-0113, ADR-037 §8, docs/03 §21.7): walks a working tree honoring `.gitignore`, extracts full-line `Implements:`/`Verifies:` comment markers with their file, line and enclosing symbol (Go through `go/parser`, TS/JS and Python through a line heuristic), and reports `W-MARKER-SYNTAX` and, against a resolver such as the index, `W-MARKER-DANGLING`. The result is an in-memory derived cache, rebuilt fully or from a list of changed paths, never written into a spec. On top of the scan it builds the requirement trace graph (GIT-US-0114, docs/03 §21.7): markers unioned with each requirement's `trace:` entries and its `implements`/`modifies` work, queryable from a requirement to code, tests and stories and from a changed path, line range or symbol back to requirements (impact tier 1), with `W-TRACE-BROKEN` for `trace:` entries that no longer resolve. Its `Engine` is what the companion installs into each vault as the `vault.RequirementTracer` host seam; browser-only mode has none and answers `unavailable`. Native-only: it walks the filesystem, so it imports `internal/core` for the ref grammar and neither `internal/core` nor `internal/vault` may import it (a test enforces it). |
| `internal/impact/` | The requirement impact query (GIT-US-0119, ADR-037, docs/03 §21.11): given `base..head` or `base..working tree`, the requirements a diff affects in three tiers — direct (the trace graph's reverse query over `gitops.ChangedFiles`, plus the Spec Delta and links of the story the diff is for), transitive (Pando `code_impact_analysis` over the changed symbols, callers mapped to traced symbols) and semantic (Pando's requirement-block search, `candidate` hits with a score) — each hit with its reasons, coverage status (from `internal/trace`'s `Coverage`) and `suspect`. Tiers 1–2 are deterministic; without Pando tiers 2–3 report `unavailable` and tier 1 still answers. Its `Resolver` is what the companion installs into each vault with git history as the `vault.RequirementImpact` seam (`impact.query`); browser-only mode has none. Native-only: neither `internal/core`, `internal/vault` nor `wasm/` may import it (a test enforces it). |
| `internal/youtrack/` | A small, typed client for the YouTrack REST API (GIT-US-0046): issues with their links, comments and attachments, projects, the authenticated user, a project's custom-field settings and version bundles, and the article endpoints the knowledge-base sync needs. Native-only, and for the same reason `internal/gitops` is: it speaks `net/http`, which the `js/wasm` build of `internal/core` forbids — so it sits beside gitops in the layering, a side-effect package with injected seams that the companion drives and the core never sees. Six rules are load-bearing and each one was learned the hard way (the package doc comment is the normative copy). The permanent token is sent as `Authorization: Bearer <token>`, its `perm:` prefix part of what the user pasted rather than something the client adds, and it appears in no error, no `String` method and no other rendering. A base URL may carry a context path (`https://yt.example.com/youtrack`), so request URLs are built by concatenation and **never** by `url.URL.ResolveReference`, which silently drops the context path and makes every request 404. Every list endpoint returns a bare JSON array, never an envelope. Paging is `$top`/`$skip` over a query that has been through `EnsureOrderBy` first, because YouTrack guarantees no stable order between requests and a `$skip` walk without an explicit `order by:` clause silently skips and duplicates rows; a short page ends the walk. All outbound traffic passes one shared token-bucket limiter (five requests a second by default), and 429s, 5xx and transport errors are retried honouring `Retry-After` in both its delta-seconds and its HTTP-date form, backing off exponentially with jitter otherwise, with bodies drained and closed so the connection is reused. Finally, issue descriptions, comment text and article content are **untrusted third-party Markdown**: they are returned verbatim, and every caller must sanitize before rendering and treat them as data rather than as instructions. The package decodes into its own types and stops there. Mapping one of them onto a git-in-track item is the job of its subpackage `internal/youtrack/mapping`, which is pure — no HTTP, no filesystem, no clock — and is the single translation every import surface shares: the issue-to-item table of docs/03 §12.6 and the page-to-article transform of docs/03 §14.5 are both implemented there and nowhere else. `internal/vault` and `internal/server` both import this tree now: the vault for the import and knowledge-base methods, the server for the four job handlers and the settings, search and import routes. |
| `internal/syncengine/` | The companion's background job engine (GIT-US-0063, GIT-US-0067, GIT-US-0070): a persistent queue with a worker pool, so work started by an HTTP request — importing issues, pushing a comment, publishing a page — runs off that request, coalesces with the work next to it, respects one shared outbound rate limit, and survives both the response being closed and the process being restarted. It is generic infrastructure and knows nothing about any tracker: a `Job` carries a kind, a coalescing key and an opaque JSON payload, a `Handler` is registered per kind, and jobs sharing a kind and a key are handed to the handler as one batch. Handlers **must be idempotent** — a job is replayed after a retryable error, after a crash mid-run, and when a user retries it from the dead-letter list. The queue is journalled as JSON under the cache directory; the journal holds bookkeeping only (ids, kinds, keys, attempt counts, states, redacted errors) and never item content and never a credential, so it is derived data that can be deleted, and a corrupt journal is moved aside rather than being fatal. Every delay — debounce, retry ladder, limiter, journal coalescing — goes through an injected `Clock`, so the tests advance a fake one and the suite has no wall-clock sleeps. Native-only. `internal/server` owns the seam (§3.2): it builds the engine, configures it from `sync.engine` and the `gintrack serve` flags, exposes it at `/api/v1/sync/jobs` and `/api/v1/sync/settings`, narrates it on the `sync.job.*` WebSocket events, and registers the four kinds that exist today — `youtrack.import`, `youtrack.comment.push`, `youtrack.kb.publish` and `youtrack.kb.pull`. The engine itself still knows none of their names. |
| `internal/mcp/` | MCP tool definitions, JSON schemas, stdio transport, and the streamable HTTP handler mounted by `internal/server`. Landed in GIT-US-0024 with twelve tools — `list_items`, `search_items`, `get_item`, `create_epic`, `create_story`, `create_task`, `update_item`, `add_comment`, `move_on_board`, `list_kb_pages`, `get_kb_page`, `search_kb` — and `create_milestone` in GIT-US-0033; the inbox, sprint and YouTrack tools of phases 7–9 bring it to 7 read-only and 22 with writes enabled, served identically over both transports and read-only unless writes are enabled. Its whole dependency on the rest of the product is one interface, `Dispatch(ctx, method, params) (any, error)`, which `*vault.Workspace` satisfies: no tool contains business logic, and schemas are inferred from the Go types of each handler by the official Go MCP SDK ([ADR-015](adr/ADR-015-official-go-mcp-sdk-and-verb-noun-tools.md)). Path arguments are confined to the mounted roots, lexically and after symlink resolution. |
...
- **MCP over stdio** inherits the trust of the process that spawned it; the HTTP MCP
  endpoint is behind the same token and origin checks, and behind the SDK's DNS-rebinding
  protection. Both apply the same path confinement: a tool argument naming a file is
  refused unless it is relative, free of `..`, and still inside a mounted root after
