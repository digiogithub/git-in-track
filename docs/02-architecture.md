...
- **`internal/gitops`** owns the *plumbing*: `DetectVCS` reads the `.jj` marker
  ...
  snapshots the user's working copy, and it refuses every write until
  `GIT-US-0041`. Its one non-jj read is `History`, which walks the git object
  store jj commits into so the metrics stay affordable
  ...
  of `GIT-US-0038` over a git backend.
...
`GIT-US-0040`; writing through it is `GIT-US-0041`, so a jj repository is still
