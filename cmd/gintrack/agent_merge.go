...
var agentDivergenceReasons = map[string]string{
	...
	"Remembrances":           "the exported backlog corpus is indexed from this setting",
	"Remembrances.KBPath":    "the corpus lives outside the repository; Pando's directory walk has no hidden-directory exclusion, so a repository root would be indexed whole",
	"Remembrances.KBWatch":   "the watcher re-writes the documents it processes without parsing their front matter, stripping the metadata the exporter wrote",
